...
handlers.index = (data, callback) => {
  ...
  if (data.method === "GET") { _helpers.HTML.generate("index", template.data, callback); }
}

// Signup
handlers.account.signup = (data, callback) => {
  // Reject any request that isn't a GET:
  data.method !== "GET" && callback(405, undefined, "html");
  const template = {
    data: {
  ...
  if (data.method === "GET") { _helpers.HTML.generate("signup", template.data, callback); }
}

// Signin
handlers.session.signin = (data, callback) => {
  // Reject any request that isn't a GET:
  data.method !== "GET" && callback(405, undefined, "html");
  const template = {
    data: {
  ...
  if (data.method === "GET") { _helpers.HTML.generate("signin", template.data, callback); }
}

// Favicon
handlers.favicon = (data, callback) => {
  data.method !== "GET" && callback(405);
  if (data.method === "GET") {
    _helpers.getStaticAsset("favicon.ico", (err, data) => {
      !err && data ? callback(200, data, "favicon") : callback(500);
    });
  }
}
...
handlers.public = (data, callback) => {
  data.method !== "GET" && callback(405);
  if (data.method === "GET") {
    const asset = data.path.replace("public/", "").trim();
    if (asset.length) {
      _helpers.getStaticAsset(asset, (err, data) => {
        !err && data ? callback(200, data, asset.split('.').pop() || "plain") : callback(500);
      });
    } else { callback(404); }
