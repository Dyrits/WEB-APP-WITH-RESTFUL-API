...
server.run = (req, res) => {
    ...
    req.on("end", () => {
        ...
        handler(data, (statusCode, payload, contentType) => {
            ...
            const contentTypes = { "json": "application/json", "html": "text/html" };
            res.setHeader("Content-Type", contentTypes[contentType]);
            // Determine the payload:
            const payloads = { "json": JSON.stringify(payload || {}), "html": payload || String() };
            let payloadString = payloads[contentType];
            // Return the response:
