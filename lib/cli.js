...
cli.responders.help = function () {
    const commands = {
        ...
        "list logs": "Get a list of all the log files (compressed only).",
...
cli.responders.listLogs = function () {
    _logs.list(true, function (error, logs) {
...
cli.responders.moreLogInfo = function (input) {
    const name = input.split("--")[1]?.trim()?.toLowerCase();
    if (name.length) {
        cli.display.lineBreak();
        _logs.decompress(name, function (error, log) {
            if (!error && log) {
                // Split into lines:
                const lines = log.split("\n");
                for (const line of lines) {
                    const object = helpers.parseJsonToObject(line);
                    if (object && JSON.stringify(object) !== "{}") {
                        console.dir(object, { colors: true });
                        cli.display.lineBreak();
                    }
                }
            } else { console.log(`No log with the name of ${name} found.`); }
        });
    }
}
